...
if ! docker ps | grep -q tcm-postgres-dev; then
...
USER_INFO=$(docker exec tcm-postgres-dev psql -U chatmanager -d chatmanager -t -A -c "
    SELECT id, api_id 
    FROM users 
...
docker exec tcm-postgres-dev psql -U chatmanager -d chatmanager -c "
    UPDATE users 
